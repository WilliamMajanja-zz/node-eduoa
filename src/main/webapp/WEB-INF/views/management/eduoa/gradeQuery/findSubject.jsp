...
<form method="post" action="${contextPath }/management/eduoa/gradequery/list" class="required-validate" 
				...
				<li>
					...
					<select name="classId" class="combox" id="w_combox_class">
						...
						<c:forEach items="${classList }" var="item">
							<option value="${item.id }" ${item.id==classId?"selected":"" }>${item.className }</option>
						</c:forEach>
					</select>
				</li>
				<li>
...
<script type="text/javascript">
	var chart1 = new FusionCharts("${contextPath }/styles/fusionchart/FusionCharts/Pie2D.swf", "ChartId", "500", "300", "0", "0");
    chart1.setXMLData("${result}");
